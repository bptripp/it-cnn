...
class LehkySparseness:
    ...
    def __init__(self, scale_samples):
        ...
        Lehky et al. (2011) Statistics of visual responses in primate inferotemporal
        cortex to object stimuli. J Neurophysiol.
        ...
        # numerically test Ef and Vf results ...
        foo = Ef + Vf**.5*np.random.randn(1000000)
        bar = Es + Vs**.5*np.random.randn(1000000)
        print(str(np.mean(foo*bar) - El) + ' should be small')
        print(str(np.std(foo*bar)**2 - Vl) + ' should be small')

        # print(self._afb)
        # print(alb)

        # numerically check mean scaling ...
        print(str(ala*bla*alb*blb - self._afa*self._bfa*self._afb*self._bfb*Es) + ' should be 0')

        # numerically check scaling of gamma samples ...
        foo = np.random.gamma(self._afb, self._bfb, 100000)
        bar = Es + Vs**.5*np.random.randn(100000)
        egg = np.random.gamma(alb, blb, 100000)
        print(str(np.mean(foo * bar) - np.mean(egg)) + ' should be small')
        print(str(np.std(foo * bar) - np.std(egg)) + ' should be small')

        # print(self._afa*self._bfa*self._afb*self._bfb*Es)
        # print(self._afa*self._bfa*self._afb*self._bfb*Es)
